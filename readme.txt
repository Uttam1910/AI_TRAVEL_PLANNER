    ...
    const fetchTripDetails = async () => {
      try {
        ...
        if (!details.tripDetails.tripPhotoURL && details.tripDetails.location) {
          ...
          console.debug("Destination GetPlaceDetails response:", response);
          if (response?.data?.places?.length > 0) {
            const placeDetails = response.data.places[0];
            if (placeDetails.photos?.length > 0) {
              // Prefer photo_reference over name if available
              const photoReference =
                placeDetails.photos[0].photo_reference || placeDetails.photos[0].name;
              const photoUrl = `https://maps.googleapis.com/maps/api/place/photo?photoreference=${photoReference}&maxheight=400&key=${import.meta.env.VITE_GOOGLE_API_KEY}`;
              console.debug("Destination photoReference:", photoReference);
              console.debug("Constructed destination photoUrl:", photoUrl);
              details.tripDetails.tripPhotoURL = photoUrl;
            } else {
              console.warn("No photos available in destination placeDetails:", placeDetails);
            }
          } else {
            console.warn("No places found for destination:", details.tripDetails.location);
          }
        }
            ...
            limit(async () => {
              try {
                if (!hotel.hotelImageURL) {
                  ...
                  console.debug(`GetPlaceDetails response for hotel (${hotel.name}):`, response);
                  if (response?.data?.places?.length > 0) {
                    const placeDetails = response.data.places[0];
                    ...
                    if (placeDetails.photos?.length > 0) {
                      const photoReference =
                        placeDetails.photos[0].photo_reference || placeDetails.photos[0].name;
                      photoUrl = `https://maps.googleapis.com/maps/api/place/photo?photoreference=${photoReference}&maxheight=400&key=${import.meta.env.VITE_GOOGLE_API_KEY}`;
                      console.debug(`Hotel (${hotel.name}) photoReference:`, photoReference);
                      console.debug(`Constructed hotel photoUrl:`, photoUrl);
                    } else {
                      console.warn(`No photos found for hotel: ${hotel.name}`, placeDetails);
                    }
                    return {
                      ...hotel,
                    ...
                    console.warn(`No places found for hotel: ${hotel.name}`);
                  }
                }
                return hotel;
              } catch (err) {
              ...
              limit(async () => {
                try {
                  if (!activity.imageUrl) {
                    ...
                    console.debug(`GetPlaceDetails response for activity (${activity.activityName}):`, response);
                    if (response?.data?.places?.length > 0) {
                      const placeDetails = response.data.places[0];
                      ...
                      if (placeDetails.photos?.length > 0) {
                        const photoReference =
                          placeDetails.photos[0].photo_reference || placeDetails.photos[0].name;
                        photoUrl = `https://maps.googleapis.com/maps/api/place/photo?photoreference=${photoReference}&maxheight=400&key=${import.meta.env.VITE_GOOGLE_API_KEY}`;
                        console.debug(`Activity (${activity.activityName}) photoReference:`, photoReference);
                        console.debug(`Constructed activity photoUrl:`, photoUrl);
                      } else {
                        console.warn(`No photos found for activity: ${activity.activityName}`, placeDetails);
                      }
                      return {
                        ...activity,
                        imageUrl: photoUrl,
                      ...
                      console.warn(`No places found for activity: ${activity.activityName}`);
                    }
                  }
                  return activity;
                } catch (err) {
      ...
      <footer className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-12 mt-16 shadow-lg">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
            <div>
              ...
              <p className="text-gray-200">
                We are a travel company dedicated to providing the best travel experiences. Explore the world with us and create unforgettable memories.
              </p>
            </div>
            <div>
              <h4 className="text-2xl font-bold mb-4 text-yellow-300">
              ...
              <ul className="text-gray-200 space-y-2">
                ...
                <li>
                  <a href="/privacy" className="hover:text-yellow-300 transition duration-300">
                    Privacy Policy
                  </a>
                </li>
              </ul>
          ...
          <div className="border-t border-gray-300 mt-10 pt-6 text-center text-gray-200">
            <p>
              &copy; {new Date().getFullYear()} Travel Company. All rights reserved.
            </p>
          </div>
        </div>
      </footer>
