...
import useGoogleMaps from '../hooks/useGoogleMaps'; // adjust the path if needed
...
const GoogleGlobe = () => {
  ...
  useEffect(() => {
    // Only initialize the map when Google Maps API is loaded
    if (!googleMapsLoaded) {
    ...
    const mapOptions = {
      center: { lat: 20, lng: 0 }, // Center of the globe
      zoom: 2,
      mapTypeId: 'satellite', // Satellite view for a realistic look
      tilt: 45, // 45° tilt activates the 3D effect
      heading: 0,
    ...
    // Ensure 3D tilt is maintained once the map is loaded
